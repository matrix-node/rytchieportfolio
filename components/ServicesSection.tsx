...
export default function ServicesSection() {
  ...
  return (
    <section id="services" className="py-20 border-b border-[#3a494a]/10 relative text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {SERVICES.map((serv) => {
            ...
            return (
              <div
                key={serv.id}
                className="p-6 border border-white/10 bg-[#15151e]/50 hover:border-[#00f5ff]/35 rounded-xl flex flex-col justify-between group transition-all h-full shadow-lg"
              >
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5 pt-2">
                    ...
                    {serv.deliverables.map((del) => (
                      <div key={del} className="flex items-start gap-2 text-xs text-[#b9caca] font-sans">
                        <span className="text-[#00f5ff] font-mono select-none mt-0.5">&gt;</span>
                        <span>{del}</span>
                      </div>
                    ))}
                  </div>
                ...
                <button
                  ...
                  className="w-full mt-6 py-2.5 border border-[#3a494a]/20 bg-black/35 hover:bg-[#00f5ff]/10 hover:border-[#00f5ff]/30 text-primary-fixed hover:text-primary font-mono text-[9px] uppercase font-bold text-center tracking-widest rounded transition-all flex items-center justify-center gap-1 cursor-pointer focus:outline-none focus:ring-1 focus:ring-primary-container"
                >
