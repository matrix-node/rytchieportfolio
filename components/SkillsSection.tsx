...
export default function SkillsSection() {
  ...
  const getComfortStyle = (level: Skill['comfortLevel']) => {
    switch (level) {
      ...
      case 'Building with it':
        return 'text-[#adc6ff] border-[#adc6ff]/25 bg-[#adc6ff]/5';
      case 'Comfortable':
        return 'text-tertiary border-secondary/25 bg-secondary/5';
  ...
  return (
    <section id="skills" className="py-20 border-b border-[#424754]/10 relative">
              ...
              <button
                ...
                className={`px-3 py-1.5 font-mono text-[9px] uppercase tracking-wider transition-all rounded-sm cursor-pointer focus:outline-none focus:ring-1 focus:ring-primary-container ${
                  activeCategory === cat
                    ...
                    : 'border border-[#424754]/25 text-on-surface-variant hover:border-[#adc6ff]/50 hover:text-primary-fixed'
                }`}
              >
                [ {cat} ]
              </button>
          ...
          {filteredSkills.map((skill, index) => {
            ...
            return (
              <motion.div
                ...
                className="group relative p-5 border border-white/10 bg-[#1c2026]/50 hover:border-[#adc6ff]/35 rounded-xl overflow-hidden transition-all text-left flex flex-col justify-between h-[180px] shadow-lg"
                style={{
                  background: 'radial-gradient(120px circle at var(--mouse-x, -100px) var(--mouse-y, -100px), rgba(173,198,255, 0.08), transparent 85%)'
                }}
              >
                <div>
                  {/* Skill header */}
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded border border-white/[0.04] bg-[#181c22] flex items-center justify-center text-primary-fixed group-hover:border-[#adc6ff]/40 transition-colors">
                        <IconComp className="w-4 h-4 text-[#adc6ff]" />
                      </div>
                      <span className="font-sans font-semibold text-on-surface text-sm tracking-tight">
                        {skill.name}
                      </span>
                    </div>
