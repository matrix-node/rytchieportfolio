...
export default function TimelineSection() {
  return (
    <section id="timeline" className="py-20 border-b border-[#3a494a]/10 relative text-left">
      <div className="max-w-7xl mx-auto px-6">
        
        {/* Section Title */}
        <div className="mb-16">
          <div className="flex items-center gap-2 mb-2">
            <Calendar className="w-4 h-4 text-[#63f7ff]" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 relative">
          ...
          <div className="hidden lg:block absolute left-1/2 top-4 bottom-4 w-px bg-[#3a494a]/20 -translate-x-1/2" />

          {/* Left Column: Technical Experience */}
          <div className="space-y-8 relative">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-8 h-8 rounded border border-secondary/20 bg-secondary/5 flex items-center justify-center text-secondary">
                <Briefcase className="w-4 h-4" />
              </div>
              <h3 className="text-lg font-bold font-sans text-primary uppercase tracking-wide">
            ...
            <div className="space-y-6">
              {TIMELINE.filter(item => item.type === 'Experience').map((exp) => (
                <div
                  key={exp.id}
                  className="p-5 border border-white/10 bg-[#15151e]/50 hover:border-secondary/35 rounded-xl transition-all space-y-3 relative group shadow-lg"
                >
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-1.5">
                    ...
                    <span className="px-2 py-0.5 bg-secondary/10 text-secondary border border-secondary/20 font-mono text-[9px] rounded-sm uppercase tracking-wide shrink-0 h-fit w-fit">
                      {exp.period}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1 pt-2">
                    {exp.skillsGained.map((skill) => (
                      <span
                        key={skill}
                        className="px-2 py-0.5 bg-white/[0.02] border border-white/[0.05] text-[#b9caca] font-mono text-[9px] rounded-sm uppercase"
                      >
                        {skill}
                      </span>
                    ))}
          ...
          <div className="space-y-8 relative">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-8 h-8 rounded border-[#00f5ff]/20 bg-[#00f5ff]/5 flex items-center justify-center text-[#63f7ff]">
            ...
            <div className="space-y-6">
              {TIMELINE.filter(item => item.type === 'Education').map((edu) => (
                <div
                  key={edu.id}
                  className="p-5 border border-white/10 bg-[#15151e]/50 hover:border-[#00f5ff]/35 rounded-xl transition-all space-y-3 relative group shadow-lg"
                >
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-1.5">
                    <div>
