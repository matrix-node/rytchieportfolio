...
export default function ServiceCard({ service, icon, onCtaClick }: ServiceCardProps) {
  return (
    <div className="p-6 border border-white/10 bg-[#1c2026]/50 hover:border-[#adc6ff]/35 rounded-xl flex flex-col justify-between group transition-all h-full shadow-lg">
      ...
      <button
        onClick={onCtaClick}
        className="w-full mt-6 py-2.5 border border-[#424754]/20 bg-black/35 hover:bg-[#adc6ff]/10 hover:border-[#adc6ff]/30 text-primary-fixed hover:text-on-surface font-mono text-[9px] uppercase font-bold text-center tracking-widest rounded transition-all"
      >
        {service.ctaText}
      </button>
    </div>
