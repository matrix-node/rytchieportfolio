...
export default function SkillCard({ skill, icon, comfortStyle }: SkillCardProps) {
  return (
    <div className="group relative p-5 border border-white/10 bg-[#15151e]/50 hover:border-[#00f5ff]/35 rounded-xl overflow-hidden transition-all text-left flex flex-col justify-between h-[180px] shadow-lg">
      <div>
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded border border-white/[0.04] bg-[#0e0e12] flex items-center justify-center text-primary-fixed">{icon}</div>
            <span className="font-sans font-semibold text-primary text-sm tracking-tight">{skill.name}</span>
          </div>
