  ...
  return (
    <div className="w-full text-left font-mono space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="p-4 bg-[#15151e]/50 border border-white/10 rounded-xl hover:border-[#00f5ff]/30 transition-all text-xs">
          <p className="text-[9px] uppercase text-outline mb-1.5">// Build Health</p>
          <p className="text-xl font-mono text-[#00f5ff] font-bold tracking-tight">99.8%</p>
        </div>

        {/* Metric 2: Experience */}
        <div className="p-4 bg-[#15151e]/50 border border-white/10 rounded-xl hover:border-[#00f5ff]/30 transition-all text-xs">
          <p className="text-[9px] uppercase text-outline mb-1.5">// Experience</p>
          <p className="text-xl font-mono text-[#d1b3ff] font-bold tracking-tight">4+ YRS</p>
        </div>

        {/* Metric 3: Latency */}
        <div className="p-4 bg-[#15151e]/50 border border-white/10 rounded-xl hover:border-[#00f5ff]/30 transition-all text-xs">
          <p className="text-[9px] uppercase text-outline mb-1.5">// Latency</p>
          <p className="text-xl font-mono text-[#fbffbb] font-bold tracking-tight">14ms</p>
        </div>
      ...
      <div className="p-4 bg-black/60 border border-cyan-500/20 rounded-xl flex flex-col justify-between min-h-[145px] relative overflow-hidden backdrop-blur-sm shadow-[0_4px_30px_rgba(0,0,0,0.1)]">
        {/* Absolute design aesthetic decoration matching mockup signature */}
        <div className="absolute top-0 right-4 flex gap-1">
          <div className="w-8 h-[1px] bg-[#00f5ff]/30"></div>
          <div className="w-4 h-[1px] bg-[#00f5ff]/30 ml-2"></div>
        </div>
        ...
        <div className="space-y-1.5 flex-1 font-mono text-[10px] text-[#849495] overflow-y-auto">
          {logs.map((log, idx) => {
            ...
            const isStatusColor = matchesService ? 'text-[#10B981]' : log.includes('sudo') ? 'text-[#00f5ff]' : 'text-[#849495]';
            return (
              <div key={idx} className="flex gap-2">
                <span className="text-[#00f5ff]/40 select-none">$</span>
          ...
          <div className="flex items-center gap-1">
            <span className="text-[#00f5ff]/40 select-none">$</span>
            <span className="w-1.5 h-3 bg-[#00f5ff] terminal-cursor inline-block" />
          </div>
        </div>
      </div>
    </div>
